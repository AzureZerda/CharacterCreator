...
def Construct_Skill(input):
    ...
    try:
        ...
        prereqs=None
    if route==0:
        choice=skill_types.Assassin_Eligibility_Skill(input.name)
    elif route==1:
        choice=skill_types.Lockpicking(input.name,input.quant)
    elif route==2:
        choice=skill_types.Magic(input.name,input.quant)
    elif route==3:
        choice=skill_types.Priest_Level(input.quant,session['character_details']['faith'])
    elif route==4:
        choice=skill_types.Craft(input.name,input.quant)
    elif route==5:
        choice=skill_types.Instruction_Ability(input.name,input.quant,SKILL_REF[input.name]['Prereq'])
    elif route==6:
        choice=skill_types.Field_Repair_Skill(input.name,SKILL_REF[input.name]['Prereq'])
    elif route==7:
        choice=skill_types.Memory_Flaw(input.name)
    elif route==8:
        choice=skill_types.Background_Flaw(input.name,input.quant)
    elif route==9:
        choice=skill_types.Weapon_Master()
    else:
        choice=skill_types.Skill(input.name,quantity=input.quant,prereqs=prereqs)
    return choice
...
def select_prebuilt():
    ...
    for skill in PREBUILTS[prebuilt]['skills']:
        ...
        if isinstance(skill_,skill_types.Memory_Flaw):
            session['skills_added']['memory_flaws']=1
            session['character_details']['flaws_added'].append(skill)

        elif isinstance(skill_, skill_types.Background_Flaw):
            session['character_details']['flaws_added'].append(skill)
